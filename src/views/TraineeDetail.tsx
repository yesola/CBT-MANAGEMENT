...
import { MOCK_TRAINEES, MOCK_HISTORY } from '../constants';
...
export const TraineeDetail: React.FC<TraineeDetailProps> = ({ traineeId, onBack }) => {
  const trainee = MOCK_TRAINEES.find(t => t.id === traineeId) || MOCK_TRAINEES[0];
  const history = MOCK_HISTORY.filter(h => h.traineeId === traineeId);
  ...
  const [category, setCategory] = useState<'OJT' | 'Requal'>('OJT');
    ...
    <div className="space-y-6 animate-in fade-in duration-500 max-w-[1400px] mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
        <div className="flex items-start gap-4">
          <button 
            onClick={onBack}
            className="mt-1 p-2 hover:bg-slate-100 rounded-xl transition-colors text-slate-500"
          >
            <ArrowLeft className="w-5 h-5" />
          </button>
          <div>
            <h2 className="text-3xl font-bold text-slate-900 tracking-tight">훈련 기록 히스토리</h2>
            <p className="text-slate-500 mt-1"><span className="font-bold text-slate-900">{trainee.name}</span>님의 일일 현장 업무 훈련 시간을 기록하고 검토합니다.</p>
          </div>
        </div>
        <button 
      ...
      <div className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm flex flex-col md:flex-row items-center gap-3">
        ...
        <select className="bg-white border border-slate-200 rounded px-3 py-2 text-sm font-medium text-slate-600 min-w-[150px] appearance-none focus:ring-1 focus:ring-blue-500">
          ...
          <option>APP E/W</option>
          <option>ARR E/W</option>
        </select>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <tbody className="divide-y divide-slate-100 text-nowrap">
                  {history.map((log) => (
                    <tr key={log.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-5 py-4 text-sm font-medium text-slate-700">{(log.duration / 60).toFixed(1)}시간</td>
                  ...
                  {history.length === 0 && (
                    <tr>
                      <td colSpan={7} className="px-5 py-20 text-center text-slate-400 italic">해당 기간의 기록이 없습니다.</td>
                    </tr>
                  )}
                </tbody>
              </table>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-lg border border-slate-200 shadow-sm">
            <div className="flex items-center gap-3 mb-6">
              <div className="animate-spin-slow">
                <div className="w-5 h-5 border-2 border-slate-300 border-t-slate-800 rounded-full" />
              </div>
          ...
          <div className="bg-[#1e293b] text-white p-6 rounded-lg shadow-md border border-slate-800">
            ...
            <button className="w-full bg-[#3b82f6]/20 hover:bg-[#3b82f6]/30 text-blue-400 py-3 rounded-lg font-bold text-sm transition-colors border border-blue-400/30">
              평가 요청하기
            </button>
          </div>
        </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select 
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 font-semibold"
                    >
                      <option value="APP E">APP E</option>
                      <option value="APP W">APP W</option>
                      <option value="ARR E">ARR E</option>
                      <option value="ARR W">ARR W</option>
                      <option value="APP G">APP G</option>
                      <option value="IC DEP">IC DEP</option>
                      <option value="GP DEP">GP DEP</option>
